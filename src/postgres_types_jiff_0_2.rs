...
impl<'a> FromSql<'a> for SqlDateTime {
    fn from_sql(_: &Type, raw: &[u8]) -> Result<SqlDateTime, Box<dyn Error + Sync + Send>> {
...
impl<'a> FromSql<'a> for SqlTimestamp {
    fn from_sql(_: &Type, raw: &[u8]) -> Result<SqlTimestamp, Box<dyn Error + Sync + Send>> {
        let v = types::timestamp_from_sql(raw)?;
        Ok(SqlTimestamp(
            Span::new()
                .try_microseconds(v)
...
impl<'a> FromSql<'a> for SqlDate {
    fn from_sql(_: &Type, raw: &[u8]) -> Result<SqlDate, Box<dyn Error + Sync + Send>> {
...
impl<'a> FromSql<'a> for SqlTime {
    fn from_sql(_: &Type, raw: &[u8]) -> Result<SqlTime, Box<dyn Error + Sync + Send>> {
