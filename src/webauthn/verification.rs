...
pub trait Verifier: fmt::Debug {
    ...
    fn get_challenge(&self, challenge: &[u8]) -> Result<Option<Challenge>, Self::Error>;

    /// Try get the public key from the persisted data store.
    fn get_public_key(&self, raw_id: &[u8]) -> Result<Option<PersistedPublicKey>, Self::Error>;
...
impl PublicKeyCredential {
    /// Verify if a public key response is valid and trusted.
    pub fn verify<V: Verifier>(&self, verifier: &V) -> Result<bool, VerificationError<V>> {
        match &self.response {
            Response::AttestationResponse(_) => self.verify_attestation(verifier),
            Response::AssertionResponse(_) => self.verify_assertion(verifier),
        }
    }

    fn verify_attestation<V: Verifier>(&self, verifier: &V) -> Result<bool, VerificationError<V>> {
        ...
        if verifier
            ...
            .is_none_or(|challenge| {
                !challenge.is_valid()
                    || !challenge.is_for(&response.client_data_json.origin)
        ...
        let key = PKey::public_key_from_der(&response.method_results.public_key)
    ...
    fn verify_assertion<V: Verifier>(&self, verifier: &V) -> Result<bool, VerificationError<V>> {
        ...
        if !challenge.is_valid() || !challenge.is_for(&response.client_data_json.origin) {
            return Ok(false);
        };
